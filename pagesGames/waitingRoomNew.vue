...
export default {
  ...
  methods: {
    ...
    async enterRoom() {
      uni.setStorageSync("questionShow", true); // Pk对战历史中，不需要弹出教学
      ...
      this.$api.user.getPkRoomFromWaitingRoom(data).then((data) => {
        ...
        setTimeout(() => {
          this.$Router.replace({ path: `/pagesGames/pkFriendBegin` }); // 跳转到游戏开始界面
        }, 400);
      });
    },
  },
